...
void DEVPROFILE_Init(xCCI_FUNC_CallbackAction SpecializedDispatch, volatile Boolean *MaskChanges)
{
	...
	CAN_Master_IOConfig.IO_GetMessage = &ZwCANb_GetMessage;
	CAN_Master_IOConfig.IO_IsMessageReceived = &ZwCANb_IsMessageReceived;
	CAN_Master_IOConfig.IO_ConfigMailbox = &ZwCANb_ConfigMailbox;
	...
	//
	SCCIM_Init(&DEVICE_RS232_Master_Interface, &RS232_Master_IOConfig, SCCI_MASTER_TIMEOUT_TICKS_MS, &CONTROL_TimeCounter);
